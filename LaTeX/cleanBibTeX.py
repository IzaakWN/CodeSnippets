...
def writeComparisonFiles(infname,outfname,labels,compile=False,cmstdr=False):
  ...
  for subdir, bibfname in [('old',infname),('new',outfname)]:
    ...
    with open(texfname,'w') as texfile:
      texfile.write(f"% Auto-generated by cleanBibTeX on {datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}\n")
  ...
  for texdir, texfname in texfnames:
    if cmstdr:
      ...
      cmds.append(f"cd {texdir} && ../../utils/tdr --preview --style={style} --noclean --temp_dir=. b {texfname}")
    else:
      auxfname = texfname[:-4]+'.aux'
      pdffname = texfname[:-4]+'.pdf'
      pdfcmd   = f"pdflatex -interaction=nonstopmode {texfname}"
      cmds.append(f"cd {texdir} && {pdfcmd} && bibtex {auxfname} && {pdfcmd} && {pdfcmd} && open {pdffname}")
  if compile:
    for cmd in cmds:
      for subcmd in cmd.split(' && '):
        if subcmd.startswith('cd '):
          print(f">>>\n>>> Change directory to '{subcmd[3:]}'...")
          os.chdir(subcmd[3:])
        else:
          print(f">>>\n>>> Executing '{subcmd}'...")
          os.system(subcmd)
