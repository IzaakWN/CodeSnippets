...
\usepackage{tikz}
\usepackage{ifthen}
\usepackage{pgfplots} % for the axis
...
%  \fill[atom] (0,0,0) circle (4pt);
%  \fill[atom] (1,0,0) circle (4pt);
%  \fill[atom] (0,1,0) circle (4pt);
%  \fill[atom] (0,0,1) circle (4pt);
%  \fill[atom] (1,1,0) circle (4pt);
%  \fill[atom] (1,0,1) circle (4pt);
%  \fill[atom] (0,1,1) circle (4pt);
%  \fill[atom] (1,1,1) circle (4pt);
...
\begin{tikzpicture}
  ...
  \def\morse{plot (\x,{\A*(1-exp(-\a*(\x-\r)))^2})}
  
  % AXIS
  \draw[->,thick] (0,-0.05*\ymax) -- (0,\ymax)
  ...
  % MORSE POTENTIAL
  \draw[very thick,blue!50!black,variable=\x,domain=\xlow:\xmax,samples=\N,smooth,name path=morse]
    \morse;
  ...
  \foreach \i in {0,...,10}{
    ...
    \draw[thick,red!50!black,name intersections={of=morse and level\i,name=E\i}]
      (E\i-1) -- (E\i-2);
    \ifthenelse{\i < 5}{ \node[right=2pt,red!50!black,scale=0.8] at (E\i-2) {$T_\i$}; }{}
    \fill[red]
  ...
  \draw[thick,black!80,decorate,decoration={coil,amplitude=4pt,segment length=4pt}]
    (0.08,\yA) -- ($(0,\yA)!0.95!(A\i)$);
  \fill[atom] (0,\yA) circle (5pt);
  \fill[atom] (A\i) circle (5pt)
