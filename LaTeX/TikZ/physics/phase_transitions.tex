...
\begin{tikzpicture}
  ...
  \fill[mylightblue,draw=mylightblue,thin,use Hobby shortcut]
    (.06*\xmax,0) -- (pe0-1) -- (pe1-1) -- (pe2-1) -- (pe3-1) --
    (pe4-1) to[out=80,in=180]
    (C) to[out=0,in=135]
    (pe4-3) to[out=-42,in=145]
    (pe3-3) to[out=-35,in=155]
    (pe2-3) to[out=-30,in=160]
    (pe1-3) to[out=-17,in=170]
    (pe0-3) to[out=-10,in=172]
    (.96*\xmax,.145*\ymax) |- (0,0);
...
\begin{tikzpicture}
  ...
  \coordinate (Ex) at ($(E|-O)$);
  \coordinate (Ey) at ($(E-|O)$);
  \coordinate (Sx) at ($(S|-O)$);
  ...
  % AXES
  \draw[->,thick] (0,\ymin) -- (0,\ymax) node[left] {$T$ [\si{\degree C}]};
  \draw[->,thick] (\xmin,0) -- (\xmax,0) node[below] {$Q$}; %[\si{J}]
