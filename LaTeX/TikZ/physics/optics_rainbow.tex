...
\begin{tikzpicture}
  ...
  \coordinate (E) at ($(D)+(\alphIII-\thetI:\L)$);  % final ray to observer
  ...
  \draw[dashed] (O) -- (B) --++ (\alphI:0.4*\R) coordinate (PB);
  \draw[dashed] (O) -- (D) --++ (\alphIII:0.4*\R) coordinate (PD);
...
\begin{tikzpicture}
  ...
  \foreach \i [evaluate={
      ...
      \s=1.3-0.7*\i/\N+0.01*int(\y>0.9*\R); % scale
              }] in {0,...,\N}{
    \coordinate (A)   at (-\L-\R,\y);                     % incident ray
    \coordinate (B)   at (\alphI:\R);                     % entry of incident
    \coordinate (C)   at (\alphII:\R);                    % internal reflection
    \coordinate (D)   at (\alphIII:\R);                   % exit of ray
    \coordinate (E\i) at ($(D)+(\alphIII-\thetI:\s*\L)$); % final ray to observer
    \draw[beam,red!\f!blue]
      (A) -- (B) -- (C) -- (D) -- (E\i);
  }
  ...
  \draw[dashed] (O) -- (P) --++ (\alphII:0.3*\R);
  \draw[dashed] (O) -- (B) --++ (\alphI:0.3*\R) coordinate (PB);
  \draw[dashed] (O) -- (D) --++ (\alphIII:0.3*\R) coordinate (PD);
  
  % ANGLES
  \draw pic["\strut$\theta_1$",draw=black,angle radius=16,angle eccentricity=1.4] {angle = PB--B--A};
  \draw pic["$\theta_1$",draw=black,angle radius=12,angle eccentricity=1.4] {angle = E--D--PD};
...
\begin{tikzpicture}
  \def\L{2}              % length of ray outside droplet
  \def\R{2}              % droplet radius
  \def\na{1.0}           % air
