...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(1.1*\L+1.1*\A,0)}]
    ...
    \foreach \i [evaluate={
      ...
      \ifodd\i
        \tick{0,-\y}{180} node[right=0,scale=0.7] {$\Delta m=-\frac{\modd}{2}$};
        \tick{0,\y}{180} node[right=0,scale=0.7] {$\Delta m=+\frac{\modd}{2}$};
      \else
        \tick{0,-\y}{180} node[right=0,scale=0.7] {$\Delta m=-\meven$};
        \tick{0,\y}{180} node[right=0,scale=0.7] {$\Delta m=+\meven$};
      \fi
    }
  \end{scope}
  ...
  % NODAL LINES
  \draw[nodal] (NP0) --++ (0.15*\L+2.1*\A,0);
  \foreach \i [evaluate={\y=\L/sqrt((\a/(\lambd*\i))^2-1)}] in {1,...,\Nlines}{
