...
# Note:
#  PowerPoint files cannot contain PDFs, but have to be converted to e.g. EMF (vector)
#  or PNF (bitmap). This script contains both conversion. To use this PDF -> EMF or PNG
#  conversion, extra libraries need to be installed (see pdf2emf and pdf2png).
#  Please also note that vector EMF from PDFs generated by ROOT might not work as expected.
# Source:
...
def pdf2emf(pdffile,tmpdir="",uniq=True,verb=0):
  ...
  # If you want to include PDFs, first install pdf2svg and Inkscape (https://inkscape.org/release/)
  #   ln -s /Applications/Inkscape.app/Contents/MacOS/inkscape /usr/local/bin/inkscape
  #   brew install pdf2svg
  # Note this might not work as expected for PDFs generated by ROOT.
...
def pdf2png(pdffile,dpi=500,tmpdir="",uniq=True,verb=2):
  ...
  # If you want to include PDFs as PNG, first install pdf2image (https://pypi.org/project/pdf2image/)
