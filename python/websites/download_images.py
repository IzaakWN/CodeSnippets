...
#   ./download_images.py -n 100 -p 20 --compile
#   ./download_images.py -n 100 -p 20 --compile -v3
import os, re
...
def format_texdoc(code,header="",verb=0):
  ...
  tex_cls_tkz = r'\documentclass[tikz,border=5pt]{standalone}'
...
def compile_tex(fname,convert=False,clean=True,verb=0):
  """Compile given LaTeX file."""
  outdir, fname = os.path.dirname(fname), os.path.basename(fname)
  ccmd = f"cd {outdir} && pdflatex -halt-on-error {fname}; rm -v *.aux *.log"
  if convert: # convert to png
    pdf = fname.replace('.tex','.pdf')
    png = fname.replace('.tex','.png')
    ccmd += f" && convert -density 1200 -trim {pdf}[0] {png}"
  if clean:
    ccmd += f"; rm -v *.aux *.log"
  if verb>=1:
    print(f">>> compile_tex: outdir={outdir!r}, fname={fname!r}")
    print(f">>> compile_tex: ccmd={ccmd!r}")
  return os.system(ccmd)
...
def scrape_post(url,**kwargs):
  ...
  with get_codes_and_images(url,verb=verb) as (texdocs, texsnips, imgs):
    ...
    for i, imgurl in enumerate(imgs,1):
      ...
      fname = f'{dname}/image_{i}.{ext}'
      with open(fname,'wb') as file:
        file.write(requests.get(imgurl).content)
    
    # WRITE TEX DOCUMENT
    for i, code in enumerate(texdocs,1):
      fname = f'{dname}/code_{i}.tex'
      with open(fname,'w') as file:
        file.write(code)
      if r'\includegraphics' not in code:
        texfiles.append(fname)
    
    # WRITE TEX CODE SNIPPET
    for i, code in enumerate(texsnips,1):
      fname = f'{dname}/code_snippet_{i}.tex'
      with open(fname,'w') as file:
        file.write(code)
      if r'\includegraphics' not in code:
        texfiles.append(fname)
...
def main(args):
  ...
  # SETTINGS
  compile = args.compile # compile LaTeX
  ...
  blacklist = ['58878','18389','42401'] # ignore these posts
    ...
    with get_SO_posts(pageurl,verb=verb) as posts:
      ...
      for url in posts:
        ...
        if match:
          ...
          texfiles += scrape_post(url,outdir=outdir,verb=verb)
          ipost += 1
          print(f">>> ")
        else:
  ...
  print (texfiles)
  if compile:
    print(f">>> Compile {len(texfiles)} LaTeX files...")
    failed = [ ]
    for fname in texfiles:
      retcode = compile_tex(fname,convert=True,verb=verb)
      if retcode>0: # failed?
        failed.append((fname,retcode))
    print(">>> Done compiling!")
    if failed:
      print(">>> Compilation/Conversion of the following files failed (return code > 0):")
      for fname, retcode in failed:
        print(f">>>  {retcode:4d}: {fname}")
...
if __name__ == '__main__':
  ...
  parser.add_argument('-c','--compile',action='store_true',help="compile LaTeX")
  parser.add_argument('-v','--verb',   type=int,nargs='?',const=1,default=0,
                                       help="set verbosity")
