...
def harvest(**kwargs):
    ...
    signal      = "XTT-S%s-M%s" % (S_exp,mass0)
    if "bbA" in filelabel:
      signal    = "ATT-S%s-M%s" % (S_exp,mass0)
    label      += "-S%s" % (S_exp)
    
    # BINWIDTH
    #binWidth    = kwargs.get('binWidth',0)
    #if binWidth: label += "-B%s" % (binWidth)
    bin         = kwargs.get('bin',0)
    if bin: label += "-B%s" % (bin)
    ...
    print color("defining nuissance parameters ...",color="green",prepend="\n>>> ")
    nuissances = [
...
#                  ).AddSyst(harvester, "CMS_xtt_dyShape_$CHANNEL_$ERA", 'shape', SystMap()(1.00))
    ...
    # EXTRACT SHAPES
    print color("extracting shapes...",color="green",prepend=">>> ")
    for channel in channels:
        filename = "%s/xtt_%s.inputs-LowMassDiTau-13TeV-28%s.root" % (DIR_datacards,channel,filelabel)
        if bin: filename.replace(".root","_B%s.root"%bin)
    ...
    # BIN STUFF
    print color("generating bin by bin...",color="green",prepend=">>> ")
    ...
    print color("setting nuisance parameter groups...",color="green",prepend=">>> ")
    ...
    # BIN NAMES
    print color("setting format bin names...",color="green",prepend=">>> ")
    SetStandardBinNames(harvester)  # formats every entry as {analysis}_{channel}_{bin_id}_{era}
    print color("generating bin names...",color="green",prepend=">>> ")
    bins = harvester.bin_set()      # generate set of bin names
    if verbosity>0:
        print color("print observation...\n",color="green",prepend="\n>>> ")
        harvester.PrintObs()
        print color("print processes...\n",color="green",prepend="\n>>> ")
        harvester.PrintProcs()
        print color("print systematics...\n",color="green",prepend="\n>>> ")
        harvester.PrintSysts()
        print color("print parameters...\n",color="green",prepend="\n>>> ")
        harvester.PrintParams()
        print "\n"
    
    # WRITER
    print color("writing datacards...",color="green",prepend=">>> ")
    MASS = "$MASS"
    if mass0 != mass: MASS = str(mass)
    datacardtxt  = "$TAG/$ANALYSIS_$CHANNEL-$BINID%s%s-$ERA-%s.txt"              % (label,filelabel,MASS)
    datacardroot = "$TAG/$ANALYSIS_$CHANNEL%s.input-LowMassDiTau%s-$ERA-%s.root" % (label,filelabel,MASS)
    print ">>>   %s"%(datacardtxt)
...
def main():
    ...
    # NO MASS SCAN
#     for S_exp in S_exps:
#       harvest(S_exp=S_exp,mass0=28,filelabel=args.filelabel)
    
    # MASS SCAN
    for mass in masses:
      for S_exp in S_exps:
        harvest(S_exp=S_exp,mass=mass,filelabel=filelabel)
    
    # BINNED DATACARDS
#     for S_exp in S_exps:
#       for bin in bins:
#         harvest(S_exp=S_exp,mass=28,bin=bin,filelabel=args.filelabel)
